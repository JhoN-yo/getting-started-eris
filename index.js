...
bot.on('messageCreate', async (message) => {
	...
	if (message.content.startsWith(`${prefix}embed1`)) {
		let embed = {
			...
			fields: [{ name: 'This is a Field', value: 'This is a Field (Value)', inline: true }],
			author: {
...
bot.on('messageCreate', async (message) => {
	...
	if (msg.startsWith(prefix + 'serverinfo')) {
		...
		let preferredLanguage =
			message.guild.preferredLocale.charAt(0).toUpperCase() + message.guild.preferredLocale.slice(1);
		...
		if (message.guild.icon)
			icon = message.guild.dynamicIconURL(message.guild.icon.startsWith('a_') ? 'gif' : 'png', 128);
