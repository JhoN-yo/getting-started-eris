...
class App extends Client {
	...
	async login() {
		var commandModule = readdirSync('./source/commands');
		commandModule.forEach(module => {
			var commands = readdirSync(`./source/commands/${module}`);
		...
		var listenerType = readdirSync('./source/listeners');
		listenerType.forEach(type => {
			var listeners = readdirSync(`./source/listeners/${type}`);
			listeners.forEach(listen => {
				const Listener = require(`../../listeners/${type}/${listen}`)
				const listener = new Listener(this);
				Logger.warn(`[${listener.name}] Listener load sucessfully`);
